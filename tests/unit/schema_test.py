...
from part_generator.api.schemas import WasherSchema, BoltSchema
...
class TestSchemas(unittest.TestCase):
    ...
    def test_bolt_schema_valid_input(self):
        ...
        bolt = BoltSchema(**data)
        self.assertEqual(bolt.diameter, 10)
        self.assertEqual(bolt.length, 50)

    def test_washer_schema_invalid_input(self):
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=-10, outer_diameter=20, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=-20, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=20, thickness=-5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=0, outer_diameter=20, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=0, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=20, thickness=0)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter="20", thickness=0)

    def test_bolt_schema_invalid_input(self):
        with self.assertRaises(ValueError):
            BoltSchema(diameter=-10, length=50)
        with self.assertRaises(ValueError):
            BoltSchema(diameter=10, length=-50)
        with self.assertRaises(ValueError):
            BoltSchema(diameter=0, length=50)
        with self.assertRaises(ValueError):
            BoltSchema(diameter=10, length=0)
        with self.assertRaises(ValueError):
            BoltSchema(diameter="10", length=50)
    
    def test_washer_schema_missing_fields(self):
        with self.assertRaises(ValueError):
            WasherSchema(outer_diameter=20, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, thickness=5)
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=20)
    
    def test_bolt_schema_missing_fields(self):
        with self.assertRaises(ValueError):
            BoltSchema(length=50)
        with self.assertRaises(ValueError):
            BoltSchema(diameter=10)

    def test_washer_schema_extra_fields(self):
        with self.assertRaises(ValueError):
            WasherSchema(inner_diameter=10, outer_diameter=20, thickness=5, color="red")
    
    def test_bolt_schema_extra_fields(self):
        with self.assertRaises(ValueError):
            BoltSchema(diameter=10, length=50, material="steel")
